...
class Field:
    ...
    def build_castle(self, wall: str, tops: tuple):
    ...
    def fill(self, wall: str, zone: str, tops: tuple):
        # validation
    ...
    def judge_castle(self):
        def dfs_gird(graph: list, start: tuple, x_lim: int, y_lim: int):
            seen = {start}
            todo = deque()
            todo.append(start)
            prev = {}  # 経路復元
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                if path and (tuple(sorted(set(path))) not in seen):
                    res.append(tuple(path))
                seen.add(tuple(sorted(set(path))))
        return tuple(res)

    def judge_zone(self, wall):
        castle = self.judge_castle()[0]

        def sfs(graph: list, start: tuple, x_lim: int, y_lim: int):
